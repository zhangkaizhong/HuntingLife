...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        UIView *contentV = [[UIView alloc] initWithFrame:CGRectMake(0, screenHeight, screenWith, 259)];
        contentV.backgroundColor = [UIColor whiteColor];
        [self addSubview:contentV];
        self.contentV = contentV;
        
        self.pickerView = [[UIPickerView alloc]initWithFrame:CGRectMake(0, 42, [UIScreen mainScreen].bounds.size.width, 259-42)];
        ...
        //盛放按钮的View
        UIView *upVeiw = [[UIView alloc]initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 42)];
        upVeiw.backgroundColor = [UIColor whiteColor];
        [contentV addSubview:upVeiw];
        //左边的取消按钮
        cancelButton = [UIButton buttonWithType:UIButtonTypeCustom];
        cancelButton.frame = CGRectMake(15, 0, 40, 42);
        [cancelButton setTitle:@"取消" forState:UIControlStateNormal];
        cancelButton.backgroundColor = [UIColor clearColor];
        [cancelButton setTitleColor:RGBMAIN forState:UIControlStateNormal];
        [cancelButton addTarget:self action:@selector(cancelButtonClick) forControlEvents:UIControlEventTouchUpInside];
        [upVeiw addSubview:cancelButton];
        
        //右边的确定按钮
        chooseButton = [UIButton buttonWithType:UIButtonTypeCustom];
        chooseButton.frame = CGRectMake([UIScreen mainScreen].bounds.size.width - 55, 0, 40, 42);
        [chooseButton setTitle:@"确定" forState:UIControlStateNormal];
        chooseButton.backgroundColor = [UIColor clearColor];
        [chooseButton setTitleColor:RGBMAIN forState:UIControlStateNormal];
        ...
        self.titleL = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(cancelButton.frame), 0, screenWith-110, 42)];
        ...
        //分割线
//        UIView *splitView = [[UIView alloc] initWithFrame:CGRectMake(0, 40, [UIScreen mainScreen].bounds.size.width, 1)];
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView
{
    if (self.pickerViewMode == DatePickerViewDateTimeMode) {
        ...
        return 2;
    }else if (self.pickerViewMode == DatePickerViewHourMode){
        return 1;
    }else if (self.pickerViewMode == DatePickerViewYearMode){
        return 1;
    }
    return 0;
}
...
- (void)pickerView:(UIPickerView *)pickerView
    ...
    if (self.pickerViewMode == DatePickerViewDateTimeMode) {
        ...
        _string =[NSString stringWithFormat:@"%.2ld",selectedHour];
    }
    else if (self.pickerViewMode == DatePickerViewYearMode){
        switch (component) {
            case 0:
