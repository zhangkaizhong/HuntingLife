...
@interface ZJScrollPageView ()<ZJContentViewDelegate>
...
- (void)commonInit {
    ...
    // 触发懒加载
    self.segmentView.backgroundColor = self.segmentStyle.segViewBackColor;
    self.contentView.backgroundColor = self.segmentStyle.contentViewBackColor;
}
...
- (void)reloadChildVcsWithNewChildVcs:(NSArray *)newChildVcs {
    ...
    [self.segmentView reloadTitlesWithNewTitles:self.titlesArray selectedIndex:0];
...
-(void)ZJScrollViewContentViewDidScrollToIndex:(NSInteger)currentIndex{
    if (self.zjdelegate && [self.zjdelegate respondsToSelector:@selector(ZJSrollViewDidScrollToIndex:)]) {
        [self.zjdelegate ZJSrollViewDidScrollToIndex:currentIndex];
    }
}
