...
- (void)setupTitles {
    ...
    for (NSString *title in self.titles) {
        ...
        if (index == _currentIndex) {
            label.font = self.segmentStyle.titleSelectedFont;
        }
...
- (void)setupScrollLineAndCover {
    ...
    ZJCustomLabel *firstLabel = (ZJCustomLabel *)self.titleLabels[0];
    CGFloat width1=[(NSString *)firstLabel.text boundingRectWithSize:CGSizeMake(1000, firstLabel.height) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.segmentStyle.titleFont} context:nil].size.width;
    CGFloat coverX = firstLabel.zj_x;
    CGFloat coverW = width1;
    ...
    if (self.scrollLine) {
        ...
        self.scrollLine.zj_centerX = firstLabel.zj_centerX;
    }
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated {
    if (_segmentStyle.isTitleCanReload == NO) {
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        currentLabel.font = weakSelf.segmentStyle.titleSelectedFont;
        ...
        if (weakSelf.scrollLine) {
            CGFloat width1=[(NSString *)currentLabel.text boundingRectWithSize:CGSizeMake(1000, currentLabel.height) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.segmentStyle.titleFont} context:nil].size.width;
            weakSelf.scrollLine.zj_width = width1;
            weakSelf.scrollLine.zj_centerX = currentLabel.zj_centerX;
        }
    ...
    if (self.segDelegate && [self.segDelegate respondsToSelector:@selector(clickBtnLabelAtIndex:)]) {
        [self.segDelegate clickBtnLabelAtIndex:_currentIndex];
    }
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    if (self.scrollLine) {
        CGFloat width1=[(NSString *)currentLabel.text boundingRectWithSize:CGSizeMake(1000, currentLabel.height) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.segmentStyle.titleFont} context:nil].size.width;
        CGFloat width2=[(NSString *)oldLabel.text boundingRectWithSize:CGSizeMake(1000, oldLabel.height) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.segmentStyle.titleFont} context:nil].size.width;
//        self.scrollLine.zj_width = width1;
//        self.scrollLine.zj_centerX = currentLabel.zj_centerX;
        wDistance = width1 - width2;
        xDistance = currentLabel.zj_centerX - oldLabel.zj_centerX;
        self.scrollLine.zj_width = width2 + wDistance * progress;
        if (progress == 0) {
            self.scrollLine.zj_width = width1;
        }
        self.scrollLine.zj_centerX = oldLabel.zj_centerX + xDistance * progress;
    }
