...
- (void)setPicUrlArray:(NSArray *)picUrlArray{
    ...
    for (int i = 0; i< _picUrlArray.count; i++) {
        ...
        imageView.hidden = NO;
        [imageView sd_setImageWithURL:obj placeholderImage:[UIImage imageNamed:@"dddd"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        ...
        imageView.frame = CGRectMake(columnIndex * (itemW + marginx), rowIndex * (itemH + marginy), itemW, itemH);
        
    }
    
    int columnCount = ceilf(_picUrlArray.count * 1.0 / perRowItemCount);
    CGFloat h = columnCount * itemH + (columnCount - 1) * marginy;
