...
#define kTableViewCellHeight 36
...
#define kSeparatorColor [UIColor colorWithRed:241/255.0 green:241/255.0 blue:241/255.0 alpha:1]
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        _rightTableView.backgroundColor = RGBCOLOR(249, 249, 249);
        ...
        //底部线条
        _bottomLine = [[UIView alloc] initWithFrame:CGRectMake(0, self.height - 1, SCREEN_WIDTH, 1)];
...
- (UIColor *)indicatorColor {
    if (!_indicatorColor) {
        _indicatorColor = RGBCOLOR(221, 221, 221);
    }
...
- (void)setDataSource:(id<LrdSuperMenuDataSource>)dataSource {
    ...
    //画出菜单
    NSArray *titlesStrings = @[@"附近",@"服务类型",@"智能排序"];
    for (int i = 0; i < _numberOfColumn; i++) {
        //backgrounLayer
...
//        }
        ...
        //indicatorLayer
        CGPoint indicatorPosition = CGPointMake((i + 1) * numberOfLine - 20, self.height/2+1);
...
- (CAShapeLayer *)createIndicatorWithPosition:(CGPoint)position color:(UIColor *)color {
    ...
    [path addLineToPoint:CGPointMake(11, 0)];
    [path addLineToPoint:CGPointMake(5.5, 6)];
    [path closePath];
    
    layer.path = path.CGPath;
    layer.lineWidth = 0.8;
    layer.fillColor = RGBCOLOR(221, 221, 221).CGColor;
...
- (void)animateIndicator:(CAShapeLayer *)indicator reverse:(BOOL)reverse complete:(complete)complete {
    ...
    if (reverse) {
        ...
        indicator.fillColor = RGBCOLOR(221, 221, 221).CGColor;
    }
    complete();
}
...
- (void)animateTableView:(UITableView *)tableview show:(BOOL)show complete:(complete)complete {
    ...
    if (show) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                weakSelf.leftTableView.frame = CGRectMake(self.origin.x, self.origin.y + self.height, SCREEN_WIDTH / 2, tableViewHeight);
                weakSelf.rightTableView.frame = CGRectMake(self.origin.x + SCREEN_WIDTH / 2, self.origin.y +self.height, SCREEN_WIDTH / 2, tableViewHeight);
            }else {
                weakSelf.leftTableView.frame = CGRectMake(self.origin.x, self.origin.y + self.height, SCREEN_WIDTH, tableViewHeight);
            }
        }];
    }else {
        WeakSelf;
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                weakSelf.leftTableView.frame = CGRectMake(self.origin.x, self.origin.y + self.height, SCREEN_WIDTH / 2, 0);
                weakSelf.rightTableView.frame = CGRectMake(self.origin.x + SCREEN_WIDTH / 2, self.origin.y +self.height, SCREEN_WIDTH / 2, 0);
            }else {
                weakSelf.leftTableView.frame = CGRectMake(self.origin.x, self.origin.y + self.height, SCREEN_WIDTH, 0);
            }
        } completion:^(BOOL finished) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        ...
        UIView *viewSele = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 3, kTableViewCellHeight)];
        viewSele.tag = 10000;
        [cell.selectedBackgroundView addSubview:viewSele];
    ...
    if (tableView == _leftTableView) {
        ...
        if (_dataSourceFlag.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:indexPath.row inColumn:_currentSelectedColumn] > 0) {
            cell.selectedBackgroundView.backgroundColor = RGBCOLOR(249, 249, 249);
            UIView *viewSele = [cell.selectedBackgroundView viewWithTag:10000];
            viewSele.backgroundColor = RGBMAIN;
            cell.textLabel.highlightedTextColor = RGBTEXT;
            ...
            UIView *viewSele = [cell.selectedBackgroundView viewWithTag:10000];
            viewSele.backgroundColor = [UIColor clearColor];
            cell.textLabel.highlightedTextColor = RGBMAIN;
...
- (void)menuTapped:(UITapGestureRecognizer *)gesture {
    ...
    if (touchIndex == _currentSelectedColumn && _isShow) {
        ...
        [self animateIndicator:_indicators[touchIndex] background:_backGroundView tableView:_leftTableView title:_titles[touchIndex] reverse:NO complecte:^{
            self.currentSelectedColumn = touchIndex;
            self.isShow = NO;
        }];
    }else {
        //弹出menu
